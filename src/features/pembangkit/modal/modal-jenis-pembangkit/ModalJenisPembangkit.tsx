...
const ModalJenisPembangkit = () => {
  ...
  const isOpen =
    modalSnapshot.isOpen && modalSnapshot.target === "modal-jenis-pembangkit";

  const tipePembangkitOptions = tipePembangkit.map(({ id, nama }) => ({
    value: id,
    label: nama,
  }));
  ...
  const onSubmit = (event?: React.FormEvent<HTMLFormElement>) => {
    ...
    formMethods.handleSubmit(async (values) => {
      await createJenisPembangkit(values);
      setIsAddData(false);
      getJenisPembangkit();
    })();
  };
...
export default ModalJenisPembangkit;
