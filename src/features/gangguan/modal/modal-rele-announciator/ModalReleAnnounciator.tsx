...
import {
  ...
  validationSchema,
  initialValues,
} from "./ModalReleAnnounciator.constant";
import { PlusCircleOutline } from "mdi-material-ui";
import { yupResolver } from "@hookform/resolvers/yup";
import { gangguanApi } from "src/api/gangguan";

const ModalReleAnnounciator = () => {
  const modalSnapshot = useSnapshot(modal);

  const [isAdddData, setIsAddData] = useState<boolean>(false);

  const { getReleGangguanList, releGangguanList, createReleGangguan } =
    gangguanApi();

  const isOpen =
    modalSnapshot.isOpen && modalSnapshot.target === "modal-rele-announciator";
  ...
  const onSubmit = (event?: React.FormEvent<HTMLFormElement>) => {
    ...
    formMethods.handleSubmit(async (values) => {
      await createReleGangguan(values);
      setIsAddData(false);
      getReleGangguanList();
...
export default ModalReleAnnounciator;
