...
import {
  ...
  validationSchema,
  initialValues,
} from "./ModalJenisGangguan.constant";
import { PlusCircleOutline } from "mdi-material-ui";
import { yupResolver } from "@hookform/resolvers/yup";
import { gangguanApi } from "src/api/gangguan";

const ModalJenisGangguan = () => {
  const modalSnapshot = useSnapshot(modal);

  const [isAdddData, setIsAddData] = useState<boolean>(false);

  const { getJenisGangguanList, jenisGangguanList, createJenisGangguan } =
    gangguanApi();

  const isOpen =
    modalSnapshot.isOpen && modalSnapshot.target === "modal-jenis-gangguan";
  ...
  const onSubmit = (event?: React.FormEvent<HTMLFormElement>) => {
    ...
    formMethods.handleSubmit(async (values) => {
      await createJenisGangguan(values);
      setIsAddData(false);
...
export default ModalJenisGangguan;
