...
const GrafikTransfer = (): JSX.Element => {
  const [date, setDate] = useState<any>(new Date());
  const { getGrafikTransfer, grafikTransfer } = grafikApi();

  const direction = "ltr";

  const getAllDataGrafik = () => {
    getGrafikTransfer({ tanggal: convertDate(date) });
  };
  ...
  const generateData = () => {
    const result = Object.values(TIME).map((time) => {
      ...
      const dataGrafik = grafikTransfer?.map(
        (value: DataGrafikPembangkit) => ({
          [value?.nama]: (value as any)[mw]! || 0,
        })
      );
  ...
  const colorGrafik = grafikTransfer?.map((value: any) => ({
    nama: value?.nama,
