...
const GrafikIBT = (): JSX.Element => {
  const [date, setDate] = useState<any>(new Date());
  const { getGrafikIBT, grafikIBT } = grafikApi();

  const direction = "ltr";

  const getAllDataGrafik = () => {
    getGrafikIBT({ tanggal: convertDate(date) });
  };
  ...
  const generateData = () => {
    const result = Object.values(TIME).map((time) => {
      ...
      const dataGrafik = grafikIBT?.map((value: DataGrafikPembangkit) => ({
  ...
  const colorGrafik = grafikIBT?.map((value: any) => ({
    nama: value?.nama,
